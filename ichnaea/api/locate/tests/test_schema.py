...
class BaseCellTest(object):
    def compare(self, name, value, expect, radio="lte"):
        if name == "radioType":
            ...
            radio = radio
        item = {name: value, "radioType": radio}
...
class TestValidCellAreaLookupSchema(BaseCellTest):
    ...
    def test_asu(self):
        ...
        assert self.sample({"radioType": "gsm", "asu": 15})["signalStrength"] == -83
        assert self.sample({"radioType": "wcdma", "asu": 15})["signalStrength"] == -101
        assert self.sample({"radioType": "lte", "asu": 15})["signalStrength"] == -125
...
class TestValidCellLookupSchema(BaseCellTest):
    ...
    def test_radio(self):
        item = self.sample({"radioType": "gsm", "cellId": constants.MAX_CID_GSM + 1})
