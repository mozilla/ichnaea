...
class TestRegionSource(BaseSourceTest):
    ...
    def test_blue(self, geoip_db, http_session, session, source, stats):
        ...
        query = self.model_query(
            geoip_db, http_session, session, stats, blues=[blue1, blue2, blue3]
        )
    ...
    def test_blue_miss(self, geoip_db, http_session, session, source, stats):
        blues = BlueShardFactory.build_batch(2, samples=10)
        session.flush()

        query = self.model_query(geoip_db, http_session, session, stats, blues=blues)
        results = source.search(query)
    ...
    def test_from_mcc(self, geoip_db, http_session, session, source, stats):
        ...
        query = self.model_query(geoip_db, http_session, session, stats, cells=[area])
        results = source.search(query)
        self.check_model_results(results, [region])
    ...
    def test_ambiguous_mcc(self, geoip_db, http_session, session, source, stats):
        now = util.utcnow()
    ...
    def test_multiple_mcc(self, geoip_db, http_session, session, source, stats):
        now = util.utcnow()
        region = GEOCODER.regions_for_mcc(235, metadata=True)[0]
    ...
    def test_wifi(self, geoip_db, http_session, session, source, stats):
        ...
        query = self.model_query(
            geoip_db, http_session, session, stats, wifis=[wifi1, wifi2, wifi3]
        )
        results = source.search(query)
        self.check_model_results(results, [region])
        best_result = results.best()
    ...
    def test_wifi_miss(self, geoip_db, http_session, session, source, stats):
