...
class LocateV0Base(BaseLocateTest):
    ...
    def model_query(self, blues=(), cells=(), wifis=()):
        ...
        if blues:
            query["blue"] = []
            for blue in blues:
                query["blue"].append({"key": blue.mac})
        if cells:
            query["cell"] = []
            for cell in cells:
                cell_query = {
        ...
        if wifis:
            query["wifi"] = []
            for wifi in wifis:
                query["wifi"].append({"key": wifi.mac})
        return query
        ...
        assert items == [
            {
                ...
                "report": {
                    "bluetoothBeacons": [
                        {"macAddress": blue_query[0]["key"], "signalStrength": -50},
                        {"macAddress": blue_query[1]["key"], "name": "my-beacon"},
        ...
        assert items == [
            {
                ...
                "report": {
                    "wifiAccessPoints": [
                        ...
                        {"macAddress": wifi_query[2]["key"], "signalToNoiseRatio": 13},
                        {"macAddress": wifi_query[3]["key"], "ssid": "my-wifi"},
                    ],
                    "fallbacks": {"ipf": True, "lacf": True},
                    "position": {
