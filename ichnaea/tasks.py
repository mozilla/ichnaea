...
def cleanup_kombu_message_table(self, ago=0):
    with get_client('ichnaea').timer("task.cleanup_kombu_message_table"):
        now = datetime.utcnow()
...
def schedule_new_moving_wifi_analysis(self, ago=1, batch=1000):
    with get_client('ichnaea').timer("task.schedule_new_moving_wifi_analysis"):
...
def blacklist_moving_wifis(self, ago=1, offset=0, batch=1000):
    with get_client('ichnaea').timer("task.blacklist_moving_wifis"):
...
def remove_wifi(self, wifi_keys):
    with get_client('ichnaea').timer("task.remove_wifi"):
...
def cell_location_update(self, min_new=10, max_new=100, batch=10):
    with get_client('ichnaea').timer("task.cell_location_update"):
        try:
            cells = []
            with self.db_session() as session:
...
def wifi_location_update(self, min_new=10, max_new=100, batch=10):
    with get_client('ichnaea').timer("task.wifi_location_update"):
        try:
            wifis = {}
            with self.db_session() as session:
