...
class TestCellObservation(BaseTest):
    ...
    def test_weight(self):
        ...
        assert round(obs_factory(
            radio=Radio.gsm, accuracy=None, signal=-95).weight, 2) == 1.0
        assert round(obs_factory(
            radio=Radio.gsm, accuracy=0.0, signal=-95).weight, 2) == 1.0
        assert round(obs_factory(
            radio=Radio.gsm, accuracy=10.0, signal=-95).weight, 2) == 1.0
        assert round(obs_factory(
            radio=Radio.gsm, accuracy=160, signal=-95).weight, 2) == 0.25
        assert round(obs_factory(
            radio=Radio.gsm, accuracy=200, signal=-95).weight, 2) == 0.22
        assert round(obs_factory(
            radio=Radio.gsm, accuracy=1000, signal=-95).weight, 2) == 0.1
        assert round(obs_factory(
            radio=Radio.gsm, accuracy=1000.1, signal=-95).weight, 2) == 0.0

        assert round(obs_factory(
            radio=Radio.gsm, accuracy=10.0, signal=-51).weight, 2) == 10.17
        assert round(obs_factory(
            radio=Radio.gsm, accuracy=160.0, signal=-51).weight, 2) == 2.54
        assert round(obs_factory(
            radio=Radio.gsm, accuracy=10.0, signal=-113).weight, 2) == 0.52

        assert round(obs_factory(
            radio=Radio.wcdma, accuracy=10.0, signal=-25).weight, 2) == 256.0
        assert round(obs_factory(
            radio=Radio.wcdma, accuracy=160.0, signal=-25).weight, 2) == 64.0
        assert round(obs_factory(
            radio=Radio.wcdma, accuracy=10.0, signal=-121).weight, 2) == 0.47

        assert round(obs_factory(
            radio=Radio.lte, accuracy=10.0, signal=-43).weight, 2) == 47.96
        assert round(obs_factory(
            radio=Radio.lte, accuracy=160.0, signal=-43).weight, 2) == 11.99
        assert round(obs_factory(
            radio=Radio.lte, accuracy=10.0, signal=-140).weight, 2) == 0.3
...
class TestCellReport(BaseTest):
    ...
    def test_lac_cid(self):
        assert self.sample(
            ...
            cid=constants.MAX_CID_GSM, psc=None) is None
        assert self.sample(
            radio=Radio.gsm, lac=None,
            cid=constants.MAX_CID_GSM, psc=1) is None

    def test_cid(self):
        for radio in (Radio.gsm, Radio.wcdma, Radio.lte):
            assert self.sample(
                radio=radio, cid=constants.MIN_CID - 1) is None
            assert self.sample(
                radio=radio, cid=12345)['cid'] == 12345
            assert self.sample(
                radio=radio, cid=constants.MAX_CID + 1) is None

        # correct radio type for large GSM cid
        cid = constants.MAX_CID_GSM + 1
        assert self.sample(
            radio=Radio.gsm, cid=cid)['radio'] is Radio.wcdma
    ...
    def test_psc(self):
        for radio in (Radio.gsm, Radio.wcdma, Radio.lte):
            assert self.sample(
                radio=radio, psc=constants.MIN_PSC - 1)['psc'] is None
            assert self.sample(
                radio=radio, psc=15)['psc'] == 15
            assert self.sample(
                radio=radio, cid=constants.MAX_PSC + 1)['psc'] is None

        assert self.sample(
            radio=Radio.lte, psc=constants.MAX_PSC_LTE + 1)['psc'] is None
    ...
    def test_asu(self):
        for radio in (Radio.gsm, Radio.wcdma, Radio.lte):
            assert self.sample(
                radio=radio,
                asu=constants.MIN_CELL_ASU[radio] - 1)['asu'] is None
            assert self.sample(
                radio=radio, asu=15)['asu'] == 15
            assert self.sample(
                radio=radio,
                asu=constants.MAX_CELL_ASU[radio] + 1)['asu'] is None
    ...
    def test_signal(self):
        for radio in (Radio.gsm, Radio.wcdma, Radio.lte):
            assert self.sample(
                radio=radio,
                signal=constants.MIN_CELL_SIGNAL[radio] - 1)['signal'] is None
            assert self.sample(
                radio=radio, signal=-75)['signal'] == -75
            assert self.sample(
                radio=radio,
                signal=constants.MAX_CELL_SIGNAL[radio] + 1)['signal'] is None
