...
def _dump_nothing(datatype, session, filename, lat=None, lon=None, radius=None):
    return 0
...
class TestDump(object):
    ...
    def _export(self, session, datatype, expected_keys, restrict=False):
        with util.selfdestruct_tempdir() as temp_dir:
            ...
            if restrict:
                dump.dump_file(
                    datatype, session, path, lat=GB_LAT, lon=GB_LON, radius=25000
                )
            else:
    ...
    def _cell_keys(self, cells):
        ...
        for cell in cells:
            keys.append(
                ",".join([cell.radio.name.upper()] + [str(c) for c in cell.cellid[1:]])
            )
        return keys
