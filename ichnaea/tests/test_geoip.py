...
GEOIP_BAD_FILE = os.path.join(DATA_DIRECTORY, "GeoIP2-Connection-Type-Test.mmdb")
...
class TestGeoIP(object):
    ...
    def test_regions(self, geoip_db):
        ...
        mapped_regions = set(
            [geoip.GEOIP_GENC_MAP.get(r, r) for r in geoip.REGION_SCORE.keys()]
        )
        assert mapped_regions - valid_regions == set()
        for region in mapped_regions:
            radius, region_radius = geoip_db.radius(region, loc, default=None)
...
class TestRadius(object):
    ...
    def test_subdivision(self, geoip_db):
        assert geoip_db.radius("RU", Location(2100))[0] > 2000000.0
        assert geoip_db.radius("RU", Location(2100), subs=["A"])[0] < 1500000.0
        assert geoip_db.radius("RU", Location(100), subs=["A"])[0] == 100000.0

    def test_city(self, geoip_db):
        assert geoip_db.radius("GB", Location(41), city=2643743)[0] > geoip.CITY_RADIUS
        assert geoip_db.radius("GB", Location(1), city=2643743)[0] == 1000.0
        assert (
            geoip_db.radius("RU", Location(26), subs=["A"], city=1)[0]
            == geoip.CITY_RADIUS
        )
        assert geoip_db.radius("LI", Location(26), city=1)[0] < geoip.CITY_RADIUS
        assert geoip_db.radius("US", Location(26), city=1)[0] == geoip.CITY_RADIUS

    def test_unknown(self, geoip_db):
        assert geoip_db.radius("XX", Location(5100))[0] == geoip.REGION_RADIUS
        assert geoip_db.radius("XX", Location(100))[0] == 100000.0
        assert geoip_db.radius("XX", Location(26), city=1)[0] == geoip.CITY_RADIUS
        assert geoip_db.radius("XX", Location(10), city=1)[0] == 10000.0
